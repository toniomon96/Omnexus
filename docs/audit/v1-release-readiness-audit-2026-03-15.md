...
| Exercise library | V1 shipped | Partially mismatched with docs | `src/data/exercises/index.ts`, `src/pages/ExerciseLibraryPage.tsx`, `src/pages/ExerciseDetailPage.tsx`, `api/exercise-search.ts`, `tests/e2e/library.spec.ts` | Discovery/detail/search are implemented, but active docs still say 316 exercises while `EXERCISE_LIBRARY.length` is currently 307. |
...
4. **Documentation accuracy is drifting.** Active docs still overstate the current exercise count and had stale test totals before this audit.
